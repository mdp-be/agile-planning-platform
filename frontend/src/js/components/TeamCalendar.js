...
console.log('DyanmicTable loaded');
...
export default class DynamicTable extends React.Component {
    ...
    getIssuesPromise() {
        return new Promise(async resolve => {
            ...
            console.log(startField + endField)


            let newTaskArray = [];
            console.log(window.location.origin)
            let response = await fetch(base_url + '/rest/api/2/search?jql=type=request AND "associated team" = "' + teamname +'"', {
                method: 'GET',
                headers: {
                    "Accept": "application/json",
                    'Content-Type': 'application/json'
            ...
            for (let obj in Array.from(data.issues)) {
                ...
                let team = data.issues[obj].fields[teamField]
...
window.addEventListener('load', function () {
    const wrapper = document.getElementById("container_team");
